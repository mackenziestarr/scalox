...
class ScanTest
  ...
  it should "ignore whitespace, newlines, and comments" in {
    ...
    scan("/* multi-line /* nested */ unterminated comment").left.value.u.map(_.show) shouldBe Vector("[line 1] Error: Unterminated multi-line comment.")
  }
  it should "parse Number" in {
    ...
    scan("20.").left.value.u.map(_.show) shouldBe Vector("[line 1] Error: Invalid numeric syntax, found '20.' expected '20.0'")
    scan(".20").left.value.u.map(_.show) shouldBe Vector("[line 1] Error: Invalid numeric syntax, found '.20' expected '0.20'")
  }
  it should "parse String" in {
    ...
    scan("\"hello, world").left.value.u.map(_.show) shouldBe Vector("[line 1] Error: Unterminated string.")
  }
  ...
  it should "error on unrecognized characters" in {
    scan("^").left.value.u.map(_.show) shouldBe Vector("[line 1] Error: Unexpected character: '^'")
  }